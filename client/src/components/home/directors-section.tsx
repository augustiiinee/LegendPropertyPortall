import { useQuery } from '@tanstack/react-query';
import { Link } from 'wouter';
import SectionHeading from '@/components/ui/section-heading';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import { ArrowRight, Briefcase, Users } from 'lucide-react';

export default function DirectorsSection() {
  // Leadership preview data for homepage
  const leadershipPreview = [
    {
      id: 1,
      name: "Boniface K. Terer",
      position: "Chief Executive Officer",
    },
    {
      id: 2,
      name: "David C. Ruto",
      position: "Director, Agency Marketing & Letting",
    },
    {
      id: 3,
      name: "Geoffrey Koros",
      position: "Director, Property Management",
    }
  ];
  
  return (
    <section id="directors" className="py-16 md:py-24 bg-white">
      <div className="container mx-auto px-4">
        <SectionHeading
          title="Our Leadership Team"
          description="Meet the experienced professionals guiding Legend Management Limited to success."
        />
        
        <div className="mt-12 max-w-4xl mx-auto">
          <Card className="border-none shadow-lg overflow-hidden bg-neutral-50">
            <CardContent className="p-8">
              <div className="flex items-center gap-3 mb-6">
                <Users className="h-6 w-6 text-primary" />
                <h3 className="font-montserrat font-semibold text-xl text-primary">Board of Directors</h3>
              </div>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {leadershipPreview.map((leader) => (
                  <div key={leader.id} className="flex items-center gap-3 group">
                    <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center text-primary flex-shrink-0 group-hover:bg-primary/20 transition-all">
                      <span className="text-xl font-bold">{leader.name.charAt(0)}</span>
                    </div>
                    <div>
                      <h4 className="font-medium text-primary">{leader.name}</h4>
                      <div className="flex items-center text-sm text-neutral-600">
                        <Briefcase className="h-3 w-3 mr-1" />
                        <span>{leader.position}</span>
                      </div>
                    </div>
                  </div>
                ))}
              </div>
              
              <div className="mt-8 pt-6 border-t border-neutral-200 flex justify-between items-center">
                <p className="text-neutral-700">
                  Our leadership team brings over 79 years of combined experience in property consulting and real estate management across Kenya.
                </p>
                <Link href="/directors">
                  <Button variant="outline" className="flex items-center gap-2 group">
                    <span>View All</span>
                    <ArrowRight className="h-4 w-4 group-hover:translate-x-1 transition-transform" />
                  </Button>
                </Link>
              </div>
            </CardContent>
          </Card>
          
          <div className="text-center mt-12">
            <p className="text-neutral-700 max-w-2xl mx-auto mb-6">
              Legend Management Limited is a premier property consultancy firm established in Kenya. 
              We specialize in providing integrated solutions in property valuation, facility management, project management, 
              and investment appraisal across Nairobi, Mombasa, Kisumu, Eldoret, Nakuru, and Nanyuki.
            </p>
            <Link href="/about">
              <Button variant="secondary" className="font-medium">
                Learn More About Us
              </Button>
            </Link>
          </div>
        </div>
      </div>
    </section>
  );
}